...
function alpha() {
   ...
   <form action="memberslist.php" method="post">
      <div class="form-group row">
         <label class="col-form-label col-sm-3" for="mblst_search">'.translate("Search").'</label>
         <div class="col-sm-9">
...
if ( $letter != 'front' ) {
   ...
   if ( $num_rows_per_order > 0 ) {
      ...
      while($temp_user = sql_fetch_assoc($result) ) {
         ...
         if(autorisation(-127) or $temp_user['uid']!= 1)
...
<div class="mt-3 card card-block-small">
   <p class=""><strong>'.translate("Mailing list").' :</strong>&nbsp;';
         if ($list) {
         echo urldecode($list);
            echo '
      ...
      if ( $num_rows_per_order > $pagesize ) {
         ...
         echo '<div class="mt-3 lead align-middle"><span class="badge badge-secondary lead">'.$num_rows_per_order.'</span> '.translate("users found").'</div>';
      }
}
   include("footer.php");
?>