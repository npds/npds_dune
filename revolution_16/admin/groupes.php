...
/*         while (list($bidon,$uidX)=each($tab_groupe)) {
            if ($uidX) {
               ...
               <li id="'.$uname.$uidX.'_'.$gp.'" style="list-style-type:none; line-height:30px;">
                  ...
                  <div>
                     <a href="admin.php?chng_uid='.$uidX.'&amp;op=modifyUser" title="'.adm_translate("Editer les informations concernant").' '.$uname.'" data-toggle="tooltip"><i class="fa fa-edit fa-lg"></i></a>
                     <a href="admin.php?op=retiredugroupe&amp;uid='.$uidX.'&amp;uname='.$uname.'&amp;groupe_id='.$gp.'" title="'.adm_translate("Exclure").' '.$uname.' '.adm_translate("du groupe").' '.$gp.'" data-toggle="tooltip"><i class="fa fa-user-times fa-lg text-danger"></i></a>';
        echo '
                  <a class="href" data-toggle="modal" data-target="#moderation"><i class="fa fa-balance-scale fa-lg"></i></a>
                  <div id="moderation" class="modal fade" role="dialog" aria-labelledby="exampleModalLong" aria-hidden="true">
                    <div class="modal-dialog modal-sm">
                      <div class="modal-content">
                         <div class="modal-header">
                             <h5 class="modal-title" id="exampleModalLongTitle">'.adm_translate("Modérateur").' '.$uname.'</h5>
                  ...
                  $res=sql_query("SELECT f.forum_id, f.forum_name, f.forum_moderator FROM ".$NPDS_Prefix."forums f WHERE f.forum_pass='$gp'");
                     ...
                     if (preg_match($pat, $row[2])) {
                        ...
                        if (count($tmp_moder)!= 0) {
                           echo'<a class=" text-danger" href="admin.php?op=moderateur_update&amp;forum_id='.$row[0].'&amp;forum_moderator='.$new_moder.'" ><i class="fa fa-user fa-lg mr-2"></i>'.adm_translate("Oter").' '.$uname.' '.adm_translate("des modérateurs du forum").' '.$row[0].'</a><br />';
                        } else {
                           echo'<i class="fa fa-user fa-lg mr-2"></i>'.adm_translate("Ce modérateur").' ('.$uname.') '.adm_translate("n'est pas modifiable tant qu'un autre n'est pas nommé pour ce forum").' '.$row[0].'';
                        }
                     } else {
                        $tmp_moder[]=$uidX;
                        asort ( $tmp_moder );
                        $new_moder=implode (',',$tmp_moder);
                        echo'<a href="admin.php?op=moderateur_update&amp;forum_id='.$row[0].'&amp;forum_moderator='.$new_moder.'"><i class="fa fa-user fa-lg mr-2"></i>'.adm_translate("Nommer").' '.$uname.' '.adm_translate("comme modérateur du forum").' '.$row[1].' ('.$row[0].')</a><br />';
                     }
                  }
                  echo '
                        </div>
