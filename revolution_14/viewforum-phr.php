...
if ( ($myrow['forum_type'] == 1) and ( ($myrow['forum_name'] != $forum_name) or ($Forum_passwd != $myrow['forum_pass'])) ) {
   ...
   echo '<p class="lead">'.translate("Moderated By: ").'';
    ...
    echo "</p>\n";
    echo '<form role="form" action="viewforum.php" method="post">';
   ...
   if ($forum_access!=9) {
      ...
      if ($allow_to_post) {
         echo '<p class="text-left"><a class="btn btn-xs btn-primary" role="button" href="newtopic.php?forum='.$forum.'" title="'.translate("New").'">';
         echo '<i class="fa fa-2x fa-pencil"></i></a></p>';
      }
   }
   ...
   echo "<td>".translate("Topic")."&nbsp;&nbsp;$mess_closoled</td>";
   echo "<td>".translate("Replies")."</td>";
   echo "<td>".translate("Poster")."</td>";
   echo "<td>".translate("Views")."</td>";
...
//               $image = $imgtmpL;
            ...
            echo '<td><i class="fa fa-lock fa-lg text-primary"></i></td>';
            ...
            } else echo "<td><img src=\"$imgtmpP\" border=\"0\" alt=\"\" /></td>\n";
            ...
            if ($Sredirection) {
               if (!$Mmod) {
                  ...
                  echo "<td>[ <a href=\"$hrefX?topic=".$myrow['topic_id']."&amp;forum=$forum\" class=\"NOIR\">".translate("Edit")."</a> ]</td>";
               }
               echo "<td>&nbsp;</td>";
            } else {
      ...
      if ($forum_access!=9)
         echo '<td>'.translate("There are no topics for this forum. ").'<br /><a href="newtopic.php?forum='.$forum.'">'.translate("You can post one here.").'</a></td></tr>';
   }
	 echo '</table>';
   if ($user) {
   ...
   echo '<div class="md-8"><i class="fa fa-file-text-o fa-lg text-primary"></i> = '.translate("New Posts since your last visit.").'</div>';
   
   echo "<div class=\"md-8\"><i class=\"fa fa-file-text fa-lg text-primary\"></i> = ".translate("More than")." $hot_threshold ".translate("Posts");
   echo "</div>";
   
   echo '<div class="md-8"><i class="fa fa-file-o fa-lg text-primary"></i> = '.translate("No New Posts since your last visit.").'</div>';
   
   echo "<div class=\"md-8\"><i class=\"fa fa-file fa-lg text-primary\"></i> = ".translate("More than")." $hot_threshold ".translate("Posts")."</div>";
   echo '<div class="md-8"><i class="fa fa-lock fa-lg text-primary"></i> = '.translate("Topic is Locked - No new posts may be made in it").'</div>';
   ...
   if (($cache_obj->genereting_output==1) or ($cache_obj->genereting_output==-1) or (!$SuperCache)) {
	...
	echo '<form class="form" role="form" action="viewforum.php" method="post">';
	echo '
