...
function readnews ($blog_dir, $op, $perpage, $startpage, $action, $adminblog) {
   ...
   if ($adminblog) {
      ...
      if (substr($action,0,1)=="A") {
         $content.='
         <form class="" role="form" name="adminForm" method="post" action="minisite.php?op='.$op.'&action=AOK">
      ...
      if (substr($action,0,1)=="M") {
         ...
         $content.='
         <form class="" role="form" name="adminForm" method="post" action="minisite.php?op='.$op.'&action=MOK&index='.$index.'">
            <div class="form-group">
               <label class="form-control-label" for="title">'.translate("Title").'</label>
