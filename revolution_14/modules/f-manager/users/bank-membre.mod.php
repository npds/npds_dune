...
if (($obj->FieldView=="jpg") or ($obj->FieldView=="gif") or ($obj->FieldView=="png")) {
   if ($tiny_mce)
      $url_modifier="\"#\" onclick=\"javascript:window.opener.tinyMCE.execCommand('mceInsertContent',true,'<img src=".str_replace(" ","%20","users_private".str_replace(dirname($basedir_fma),"",$cur_nav_back)."/".basename($cur_nav)."/".$obj->FieldName)." border />');\"";
   else
      $url_modifier="\"#\"";

} else {
   if ($tiny_mce)
      $url_modifier="\"#\" onclick=\"javascript:window.opener.tinyMCE.execCommand('mceInsertContent',true,'<a href=".str_replace(" ","%20","users_private".str_replace(dirname($basedir_fma),"",$cur_nav_back)."/".basename($cur_nav)."/".$obj->FieldName)." target=_blank>".$obj->FieldName."</a>');\"";
   else
