   ...
   case 'Submit':
      if (!$sformret) {
         ...
         if (!R_spambot($asb_question, $asb_reponse, $message)) {
            ...
            global $notify_email;
            send_email($notify_email,aff_langue($m->title),aff_langue($message),"","","html");
            opentable();
            echo "<p align=\"center\">".aff_langue("[french]Votre demande est prise en compte. Nous y r&#xE9;pondrons au plus vite[/french][english]Your request is taken into account. We will answer it as fast as possible.[/english]")."</p>";
            closetable();
            break;
         }
      } else {
