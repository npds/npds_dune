...
if ($submitS) {
   ...
   echo "<h4>".translate("Moderated By: ")."";
   for ($i = 0; $i < count($moderator); $i++) {
      echo "<a href=\"user.php?op=userinfo&amp;uname=$moderator[$i]\" class=\"box\">$moderator[$i]</a></h4>";
      if (isset($user))
   ...
   if ($allow_to_reply) {
     ...
     if ($smilies) {
		...
		echo "HTML : ";
     if ($allow_html==1) {
        echo translate("On")."<br />"; //<br /> nécéssaire ? phr
        echo HTML_Add();
     } else
        echo translate("Off")."<br />"; //<br /> nécéssaire ? phr
     if ($citation && !$submitP) {
        ...
        if ($r = sql_query($sql)) {
           ...
           if ($m['post_time']!="" && $m['uname']!="") {
              $reply = '<blockquote><p>'.translate("Quote").' : <strong>'.$m['uname'].'</strong><br />'.$text.'</p></blockquote>';
           } else {
              $reply = ''.$text.'';
           }
     ...
     echo "".Q_spambot().""; 
		echo'
			<div class="btn-group-sm text-center" role="group">
				...
				<button class="btn btn-primary" type="submit" value="'.translate("Submit").'" name="submitS" accesskey="s">'.translate("Submit").'</button>
				...
				<button class="btn btn-danger" type="submit" value="'.translate("Cancel Post").'" name="cancel">'.translate("Cancel Post").'</button>
			</div>';
   } else {
     echo "<h4>".translate("You are not allowed to reply in this forum")."</h4>";
   }
   echo '</div></form>';
		...
		<div class="row">
         ...
         if ($smilies) {
            if ($posterdata['user_avatar'] != '') {
               ...
               echo '<p><img class="img-thumbnail" src="'.$imgtmp.'" alt="'.$posterdata['uname'].'" /></p>';
            }
         }
		echo '
		</div>
