<?php
/************************************************************************/
/* Theme for NPDS / Net Portal Dynamic System                           */
/* =====================================================================*/
/*                                                                      */
/* Theme : "npds-boost" version 2015 par jpb/phr                        */
/* This theme use the NPDS theme-dynamic engine (DynaMot)               */
/*                                                                      */
/* This program is free software. You can redistribute it and/or modify */
/* it under the terms of the GNU General Public License as published by */
/* the Free Software Foundation; either version 2 of the License.       */
/************************************************************************/

$PAGES['index.php']['blocs']="0";
$PAGES['user.php']['blocs']="0";
$PAGES['memberslist.php']['blocs']="0";
$PAGES['searchbb.php']['blocs']="0";
$PAGES['article.php']['blocs']="0";
$PAGES['download.php']['blocs']="0";
$PAGES['submit.php']['blocs']="0";
$PAGES['faq.php']['blocs']="0";
$PAGES['top.php']['blocs']="0";
$PAGES['admin.php']['blocs']="0";
$PAGES['modules.php?ModPath=links&ModStart=links*']['blocs']="0";
$PAGES['modules.php?ModPath=links/admin&ModStart=links*']['blocs']="0";
$PAGES['modules.php?ModPath=f-manager&ModStart=f-manager*']['blocs']="0";
$PAGES['modules.php?ModPath=comments&ModStart=reply*']['blocs']="0";
$PAGES['modules.php?ModPath=contact&ModStart=contact']['blocs']="0";
$PAGES['modules.php?ModPath=archive-stories&ModStart=archive-stories*']['blocs']="0";
$PAGES['modules.php?ModPath=f-manager&ModStart=pic-manager*']['blocs']="0";
$PAGES['forum.php']['blocs']="0";
$PAGES['viewforum.php']['blocs']="0";
$PAGES['viewtopic.php']['blocs']="0";
$PAGES['viewtopicH.php']['blocs']="0";
$PAGES['reply.php']['blocs']="0";
$PAGES['replyH.php']['blocs']="0";
$PAGES['newtopic.php']['blocs']="0";
$PAGES['topicadmin.php']['blocs']="0";
$PAGES['editpost.php']['blocs']="0";
$PAGES['reviews.php']['blocs']="0";
$PAGES['abla.php']['blocs']="0";
$PAGES['replypmsg.php']['blocs']="0";
$PAGES['readpmsg.php']['blocs']="0";
$PAGES['map.php']['blocs']="0";
$PAGES['pollBooth.php']['blocs']="0";
$PAGES['powerpack.php']['blocs']="0";


$PAGES['admin.php']['blocs']="0";
// Extension admin.php pour REvolution
$PAGES['admin.php?op=lnl_Add_Header']['blocs']="0";
$PAGES['admin.php?op=lnl_Add_Body']['blocs']="0";
$PAGES['admin.php?op=lnl_Add_Footer']['blocs']="0";

$PAGES['user.php']['TinyMce']=1;

?>